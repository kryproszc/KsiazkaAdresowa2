...
def calculate_ratios_vectorized(df: pd.DataFrame) -> pd.DataFrame:
    if df.shape[1] < 3:
        raise ValueError("Za mało kolumn – potrzebne minimum 3 (AY + 2 wartości).")
...
class UserSessionData:
    def __init__(self):
        ...
        self.current_triangle = reactive.Value()  # lub "triangle_incurred"
        ...
        self.dev_selected_history = reactive.Value([])  # historia wyborów
        self.dev_all_initial = reactive.Value(None)  # domyślne dev_all
        ...
        self.comparison_history_i = reactive.Value([])
        ...
        # === Dopasowanie krzywych (Paid / Incurred oddzielnie jeśli trzeba)
        ...
        self.final_vector_external_incnc = reactive.Value([])

        self.comparison_history_inc = reactive.Value([])  # historia porównań INC
    ...
    def reset_incurred_state(self):
        ...
        self.dev_selected_history_i.set([])
...
def server(input, output, session):
    ...
    current_volume_inc = reactive.Value(None)  # Dla Incurred
    volume_versions_inc = reactive.Value({})
    selected_final_index_inc = reactive.Value("dev_all")  # Domyślny final index
    final_vector_external_inc = reactive.Value([])

    user_data.current_volume = current_volume
    user_data.volume_versions = volume_versions
    user_data.selected_final_index = selected_final_index
    user_data.final_values = final_values
    user_data.final_vector_external = final_vector_external
    user_data.table_dev_history_trigger = table_dev_history_trigger

    user_data.current_volume_inc = current_volume_inc
    user_data.volume_versions_inc = volume_versions_inc
    user_data.selected_final_index_inc = selected_final_index_inc
    user_data.final_vector_external_inc = final_vector_external_inc
    ...
    async def load_data(file_input, sheet_select,
                        start_row, start_col, end_row, end_col,
                        triangle_type, cumulative_option=None):
        try:
            if start_row > end_row or start_col > end_col:
                raise ValueError("Błąd: Wiersz/kolumna początkowa większa niż końcowa.")

            if (end_row - start_row + 1) < 2 or (end_col - start_col + 1) < 2:
                raise ValueError("Zakres danych jest zbyt mały. Wybierz co najmniej 2 wiersze i 2 kolumny.")
            ...
            if triangle_type == "triangle_paid":
                user_data.reset_paid_state()  # ← poprawione
                ...
                user_data.reset_incurred_state()  # ← poprawione
                old_data = user_data.reactive_data_incurred_list.get()
                user_data.reactive_data_incurred_list.set(old_data + [data])
                user_data.current_triangle.set("triangle_incurred")

            elif triangle_type == "triangle_exposure":
                old_data = user_data.reactive_data_exposure_list.get()
                user_data.reactive_data_exposure_list.set(old_data + [data])
            ...
            await hide_spinner()
            show_info_modal(f"Błąd przy wczytywaniu danych: {e}", error=True)
    ...
    def on_calculate_button_click_inc():
        ...
        # Zaktualizuj słownik wersji
        volume_dict = volume_versions_inc.get()
        volume_dict[vol] = version
        volume_versions_inc.set(volume_dict)
    ...
    def table_dev_history_inc():
        ...
        current_finals = user_data.final_vector_external_incnc.get()
    ...
    def handle_final_selection_change_inc():
        ...
        if selected == "dev_all":
            user_data.final_vector_external_incnc.set(list(user_data.dev_all_initial_i.get()))
        else:
            idx = int(selected)
            history = user_data.dev_selected_history_inc.get()
            if history and idx < len(history):
                _, values = history[idx]
                user_data.final_vector_external_incnc.set(list(values))
    ...
    def export_history_button_inc():
        ...
        final_values_list = final_vector_ui or user_data.final_vector_external_incnc.get()
    ...
    def reset_volume_history_inc():
        ...
        volume_versions_inc.set({})
        selected_final_index_inc.set("dev_all")

        dev_all = user_data.dev_all_initial_i.get()
        if dev_all is not None:
            user_data.final_vector_external_incnc.set(list(dev_all))
    ...
    def _update_final_vector_inc():
        ...
        final_vector_external_inc.set(final_vector)

    @output
    @render.ui
    def final_coefficients_display_inc():
        current_finals = final_vector_external_inc.get()
    ...
    def r2_df_freq_lc_interaktywny_inc():
        ...
        list_curve = ['Exponential', 'Weibull', 'Power', 'Inverse Power']
        f_curves_graph_real_choose = calculator.sim_data_curve_rezerwy(xs, list_curve, parameters_curve)
    ...
    def plot_curve_vs_final_inc():
        final = final_vector_external_inc.get()
    ...
    def Final_Factor_inc():
        final_factors = final_vector_external_inc.get()
        if not final_factors:
    ...
    def handle_curve_row_click_inc():
        ...
        final_vec = final_vector_external_inc.get()
    ...
    def update_tail_vector_inc():
        ...
        value = float(clicked["value"])
        num_real = input.num_of_first_factor_inc() or 0
        total_len = len(final_vector_external_inc.get())
    ...
    def export_final_vector_inc():
        ...
        # 📦 Pobierz dane
        final = final_vector_external_inc.get() or []
    ...
    def update_available_projections_i():
        history = user_data.dev_selected_history_i.get()
    ...
    async def triangle_projection_volume_inc():
        ...
        factor_map = {
            ...
            "FINAL": final_vector_external_inc.get(),  # ← wybrany FINAL
        ...
        for lbl in selected_labels:
            ...
            if vec is not None and len(vec) > 0:
                fwd = calculator.triangle_forward(df_triangle.drop(columns="AY"), vec, 1)
    ...
    def auto_calculate_projection_if_needed():
        ...
        if paid_col and paid_col not in paid_proj and paid_col in paid_factors and triangle_paid is not None:
            ...
            df_proj = calculator.triangle_forward(triangle_paid, vec, 1)
        ...
        if incurred_col and incurred_col not in inc_proj and incurred_col in incurred_factors and triangle_inc is not None:
            ...
            df_proj = calculator.triangle_forward(triangle_inc, vec, 1)
