...
import { useTableStore } from '@/stores/tableStore'; // <- Dodaj import useTableStore

export function PaidViewMultStoch() {
  const userId = useUserStore((s) => s.userId);

  // Dodane: subskrybujemy na aktualne dane z tableStore
  const tableSheetJSON = useTableStore((s) => s.selectedSheetJSON);
  const tableSheetName = useTableStore((s) => s.selectedSheetName);
  ...
  // <- Nowy efekt: synchronizacja, gdy zmienia się tableStore
  useEffect(() => {
    if (tableSheetJSON) {
      hydrateFromTableStore();
    }
  }, [tableSheetJSON, hydrateFromTableStore]); // <- teraz słucha zmian danych
