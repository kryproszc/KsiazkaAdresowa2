...
export default function WspolczynnikiMultiplikatywna(): React.ReactElement {
  ...
  const [version, setVersion] = useState(0); // 🔁 do wymuszenia renderu
  ...
  const handleSubmit = async () => {
    try {
      ...
      setSd(data.sd || []);
      setSigma(data.sigma || []);
      setVersion((v) => v + 1); // 🔁 wymusza rerender
    } catch (err) {
    ...
    <div className="p-6 text-white">
      <div className="flex gap-6">
        ...
        <div className="flex-1">
          ...
          <TrainDevideTable data={trainDevide} selected={selectedCells} onClick={toggleCell} />
              ...
              <table className="min-w-full table-auto border-collapse">
                ...
                <tbody>
                  {[{ label: 'Dev', data: dev }, { label: 'SD', data: sd }, { label: 'σ', data: sigma }].map((row, i) => (
                    <tr key={i}>
                      <td className="px-4 py-2 border font-bold text-slate-400 bg-slate-800">{row.label}</td>
                      {row.data.map((val, j) => (
