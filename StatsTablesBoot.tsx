...
function SimResults() {
  const { stats, quantileResult, percentileMatch, percentileInputSim } = useBootStore();
  if (!quantileResult) return null;

  return (
    <div className={tableBox}>
      <h4 className="font-semibold mb-3">Statystyki i kwantyle: Ultimate (boot)</h4>
...
function SimDiff() {
  const { stats, quantileResult, percentileMatch, percentileInputDiff } = useBootStore();
  if (!quantileResult) return null;

  return (
    <div className={tableBox}>
      <h4 className="font-semibold mb-3">Statystyki i kwantyle: Ultimate – przekątna (boot)</h4>
      <div className="flex flex-col lg:flex-row gap-6">
        <table className={tableStyle}>
          <thead>
            <tr>
